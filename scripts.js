const Modal = {
    ...
    close() {
        ...
        // fechar o modal
        // remover a classactive do modal
        document      // o document é a DOM
            .querySelector('.modal-overlay')
            .classList
            .remove('active')
    }
...
// Depois eu preciso somar asi saidas
...
const Transaction = {
    ...
    incomes() {
        let income = 0;                        // pegar todas as transaçoes 
        Transaction.all.forEach(transaction => {  // para cada transaçao 
...
return income;                                  // retornar essa variavel que foi criada 
...
const Form = {
    ...
    validateFields() {
        ...
        if(description.trim() === "" || // trim serve para fazer limpeza dos espaços vazio
        amount.trim() === "" || 
        date.trim() === "") {
    ...
    submit(event){
      event.preventDefault()
       //verificar se todas as informaçoes foram escritas

       try {
        Form.validateFields()
        const trasaction = Form.formatvalues()
        Transaction.add(trasaction)
        Form.clearFields()
        Modal.close()
